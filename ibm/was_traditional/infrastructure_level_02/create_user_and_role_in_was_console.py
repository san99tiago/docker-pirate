...
# Command:
# wsadmin.sh -lang jython -f /tmp/monitor.py "was_monitor" "1234" "monitor" "WAS" "MONITOR"
...
if(len(sys.argv) < 5):
    print("\n***** This script " + "configure_was_monitor.py" + " will be skipped because it expects more arguments *****")
    os._exit()
...
try:
    print("\n***** Creating/validating user %s *****" % (user_id))
...
if(len(role_already_attached) > 0):
    print("\n***** The user %s already has role %s *****" % (user_id, user_role))
else:
    # Map the created user to the "monitor" role in the 0=defaultWIMFileBasedRealm
    print("\n***** Mapping user %s to role %s *****" % (user_id, user_role))
