...
def getNodeId(prompt):
    ...
    if (len(nodeList) == 1):
        ...
        nodeName = getValidInput(
            prompt+" ["+DefaultNode+"]:", DefaultNode, nodeNameList)
...
def getServerId(prompt):
    ...
    if (len(serverList) == 1):
        ...
        serverName = getValidInput(
            prompt+" ["+DefaultServer+"]:", DefaultServer, serverNameList)
...
AdminTask.setGenericJVMArguments(
    '[-nodeName ' + node + ' -serverName ' + server + ' -genericJvmArguments "-Xnoloa"]')
